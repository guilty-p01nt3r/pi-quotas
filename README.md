...
| OpenCode Go    | Rolling 5h, weekly, monthly                                  | Account-wide usage percents and reset times from OpenCode's usage API, keyed by the `opencode-go` API key from `pi /login`; local spend tracking via the `/tokens` command |
...
- `opencode-go` — OpenCode Go API key from `pi /login opencode-go` (also reads `OPENCODE_API_KEY`). Usage comes from OpenCode's `GET /zen/go/v1/usage` endpoint.
